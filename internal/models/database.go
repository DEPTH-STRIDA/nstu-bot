...
// Возвращает определенную группу
func (db *DataBaseStruct) GetGroup(id uint) (*Group, error) {
	var group Group
...
// Устанавливает пользователю принадлежность к определенной группе
func (db *DataBaseStruct) JoinGroup(userId, groupId uint) error {
...
// Снимает принадлежность пользователя к какой-либо группе
func (db *DataBaseStruct) ExitGroup(userId uint) error {
	var user User
