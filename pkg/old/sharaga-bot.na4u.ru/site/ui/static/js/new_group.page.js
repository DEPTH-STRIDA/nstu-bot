...
//Отключение перезагрузки страницы при нажатии enter в input.
...
//Запрос на создание группы / блокировка поля ввода.
function setNewGroup() {
    ...
    var params = "&id=" + tg.initDataUnsafe.user.id + "&groupName=" + inputValue+ "&token="+tg.initData
...
//Мини функция. Возвращает название дня недели по номеру (rus)
...
function editShedule() {
    ...
    var params = "schedule=" + encodeURIComponent(schedule_json) + "&id=" + tg.initDataUnsafe.user.id + "&groupName=" + groupName+ "&token="+tg.initData
...
//разбиение массивов на строки Json
